...
if ($action === 'new') {
    $queryStr = sprintf('INSERT INTO %1$s.CLIENT (name, description) VALUES (?,?)', $db_handle->dbName);
	$stmt = $db_handle->prepareQuery($queryStr);
	$stmt->bind_param("ss", $client_name, $client_desc);
    $success_msg = 'Klient [' . $client_name . '] został dodany.';
} else {
    $queryStr = sprintf('UPDATE %1$s.CLIENT set name = ?, description = ? WHERE id = ?', $db_handle->dbName);
	$stmt = $db_handle->prepareQuery($queryStr);
...
if ($result){
	...
	$createClient = new CreateClientFolder($src, $dest, $client_name);
